...
3. Fazit, Ausblick und weiter Konfiguration
...
~~~{.groovy .numberLines stepnumber=6 frame=single captionpos=b caption="Jenkinsfile zum Compilieren, Testen und Packen von Tomcat mit Maven"}
pipeline {
	agent any
	stages {
		stage('clean') {
...
pipeline {
	...
	stages {
		...
		stage('copying_files') {
			steps {
				sh 'cp "~/workspace/Tomcat/tomcat/target/
				tomcat-6.0.5-jar-with-dependencies.jar"
...
angelegten Jenkins-Pipelines getätigt haben, anhand von Screenshots gezeigt und anschließend
genauer erläutert.
...
![Überblick der Laufzeiten des Jenkins-Jobs **Tomcat**](pictures/stage_view.png){width=75%}
...
Als letztes haben wir den Pfad zum Pipeline-Skript angegeben (Jenkinsfile_CI_build).
...
# Fazit, Ausblick und weitere Konfigurationen
...
Aufgrund des zeitlichen Rahmens konnten nicht alle gewünschten Tätigkeiten zur noch besseren Automatisierung und
...
Die Firma Red Hat bietet mit Ansible Playbooks[^11] eine Software an, um Artefakte (war, jar) automatisiert zu deployen und eine Übersicht über
alle verbreiteten Anwendungen zu erhalten.

[^11]: Mehr Informationen hier: [http://docs.ansible.com/ansible/latest/playbooks.html](http://docs.ansible.com/ansible/latest/playbooks.html)
...
mit welchem sich Tasks (wie z.\, B. assembly) manipulieren lassen, um somit einen selbst definierten Namen zu setzen.
...
Hiermit ist gemeint, dass Artefakte nach dem Bauen lediglich in einen Ordner verschoben werden und ggf. überschrieben
...
über alle gebauten Artefakte zu erhalten. Hierzu könnte z. B. die quelloffene Software *Archiva*[^12] der Apache Software Foundation genutzt werden.
Dieses Programm ist ein Webserver, an welchen Artefakte übergeben werden können, um diese zu archivieren.

[^12]: [https://archiva.apache.org/index.cgi](https://archiva.apache.org/index.cgi)   
